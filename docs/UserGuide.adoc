...
The `Find Match` button will appear when all attributes are filled. When you click this button, `ORGANice` will add the patient to the list and it will execute the `Match` command as seen in `Figure 10`.
Refer to <<Detecting matches: `match`>> for the `Match` command. Note that this button only appeared if you are adding a patient.
