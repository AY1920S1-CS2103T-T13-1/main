...
After entering the name, the patient's name will be displayed on the form as seen in `Figure 5` and the progress bar will increase indicating your progress in filling up the form.
...
Three changes occur to ORGANice, which can be seen in the screenshot below:
...
The figure below will show how the form will look like after you have entered all the required attributes.
...
you have successfully added a new person into the list as seen in `Figure 8`.
...
and you need to find potential donors for a patient as seen in `Figure 9`.
...
In the `Figure 11`, a user might have entered a wrong `Patient's` name and thus typed in `/undo` and the name that is previously reflected on the form is removed.
