...
Hence, this donor or patient do not need to be in ORGANice anymore.
...
* A default task list will be generated to show the necessary tasks the hospital need to do for the respective donor and patient.
...
A tick would mean that the task is completed and a cross will mean that the task is not completed.
...
. The patient and donor statuses will be marked to `Not Processing`.
