...
a specific donor and patient. This command serves to allow you to keep track of the necessary tasks needed to do for the patient and donor pair before the cross-matching test. +
...
* Generate a default list automatically for newly processed patient and donor
* View the list which belongs to the patient and donor pair who are currently being processed.
* Mark a task as done in the list generated from the patient and donor pair using another command `processingMarkDone ic/**PATIENT NRIC** ic/**DONOR NRIC** **TASK NUMBER**`

Some things to note for this command:
...
* The list can be updated using another command,`processingMarkDone ic/**PATIENT NRIC** ic/**DONOR NRIC** **TASK NUMBER**` +
...
The order of the NRICs does not matter. However, the NRICs must belong to a valid patient and donor. +
...
Format: `done ic/**PATIENT NRIC** ic/**DONOR NRIC**`
The order of the NRIC does not matter. However, the NRICs must belong to a valid patient and donor.

After typing this command, the patient and donor pair will be removed from ORGANice and will not be considered a potential match in future match detection.
...
* `done ic/S7896432A ic/S4578902G`
...
An extension to our current `processing` command. Future versions may be able to email the doctor in charge of a patient when an organ match for the patient is found.
