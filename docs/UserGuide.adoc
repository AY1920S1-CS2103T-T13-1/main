...
For example, when adding a patient, the display box will first display a message, prompting the user to enter the patient's name.
To add a patient that is named "John Doe", type `John Doe` and then press enter. After entering, the patient's name will be displayed in the form, as shown below.
