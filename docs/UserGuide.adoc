...
This section will explain how to run kidney matching tests on all patients stored in ORGANice and illustrate the process
with an example.
If you want to match all the patients in ORGANice,
...
determine which patient to prioritize.
...
* compatibility rate: refers to a number which indicates the degree of compatibility between the patient and donor. This percentage is calculated from the number of tissue matches between the patient and donor.
