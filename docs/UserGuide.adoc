...
That is all the important commands for this feature, now let's see an example on how the feature will look like.
When you want to add a `Patient`, type `add t/patient` on the `Command Box` and a form will appear as below and the `Result Box` will prompt the user to enter the patient's name.

image::addForm.png[width="790"]

If you want to add a patient named "John Doe", type `John Doe` and then press enter.
After entering the name, the patient's name will be displayed on the form as seen below and the progress bar will increase indicating the your progress in filling up the form.

image::nameFilled.png[width="790"]

Input for each attribute will be checked, if it is a valid input then the user input will automatically be reflected
on the form, if it is not valid, ORGANice will show a warning to you.

The following screenshot shows a warning that ORGANice will give to you if your input is not valid:

image::invalidInput.png[width="790"]

After you have entered a particular attribute and realized that you have made a mistake you can type in `/undo` to remove the last entry you entered.
In the example below, a user might have entered a wrong `Patient's` name and thus typed in `/undo` and the name that is previously reflected on the form is removed.

image::undoCommand.png[width="790"]

After all of the attributes have been specified, you can type `/done` to add the person to the ORGANice list of persons,
`/abort` if you decided not to add the person into the list, or click the `Find Match` button to directly add the person and
find the potential donors at the same time.
Note that the `Find Match` button will only appear on the `Patient` form.

image::allFieldSpecified.png[width="790"]

When you typed in `/done` the UI will changed back to it's original state and shows a success message suggesting that
you have successfully added a new person into the list.

image::addSuccess.png[width="790"]

If you decided not to proceed with the addition and typed in `/abort`, you will be directed into the main page and notified that
you have abandoned the form.

image::abandonForm.png[width="790"]

If you click the `Find Match` button instead, a list of potential donors will be reflected to you. This makes it convenient for you if you are facing an emergency situation
and you need to find potential donors for a patient.
