...
==== Listing all persons
Simply use `list` and ORGANice will show you all patients, doctors and donors present in the system.

Format: `list`

==== Listing persons of a certain type
If you wish to see a certain type of persons only, you can provide the type parameter in the list command.

Optional parameter: Type - `t/TYPE`

For instance, if you only want to see all doctors, you can enter the list command in the following format:
...
The format is that after the `edit` keyword, provide the person's NRIC, followed by the attribute(s) that you want to change in the edit command.
...
like this: `edit S8732457G p/91234567`.


Note that **you can only make changes to the attribute only if the person has that attribute**.
For example, you cannot change a doctor's age because a person of type doctor does not have that attribute in ORGANice.
You may refer to <<List of Attributes>> to see what attributes are applicable to each type of person in ORGANice.
...
These categories will be further explained below.

Note that **only one category can be used in one sort command**.
