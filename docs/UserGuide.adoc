...
If you want to delete a person in ORGANice you can type in a command in the following format: `delete **NRIC**`  +
ORGANice will prevent you from deleting persons in the following categories:

* They are a patient and their status is currently under processing.
* They are a donor and their status is currently under processing.
* They are a doctor and they are in charge of some number of patients in ORGANice.

If you want to delete a patient that has NRIC number of S4612296H and is not currently under processing, type the following command: `delete S4612296H`

// end::delete[]
=== Clearing all entries : `clear`

You can clear all entries in ORGANice by typing clear. You should run clear to start a fresh version of ORGANice after you have familiarised yourself with the commands using our dummy data.
