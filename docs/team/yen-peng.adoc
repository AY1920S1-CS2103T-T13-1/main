...
The purpose of this project portfolio is to provide an overview of my main contributions to ORGANice.
My role in this project is to design and write the code for the `list` and `sort` features.
...
This application is targeted at hospital administrators to help ease their workload by
...
* `Text#text` -- Purple texts with a hashtag in-between means Class#method, indicating that the Class contains that method.
