...
This feature is facilitated by the `MatchCommand` class, which extends the `Command` class.
This class implements the operation `MatchCommand#match(Person donor, Patient patient)`.
It checks if the given donor and patient have compatible
blood and tissue types.

The following methods in `ModelManager` accesses the `match` method:
...
`MatchedDonor` and `MatchedPatient` objects are initially stored in an `ObservableList` called
`listOfMatches`. Objects in the `listOfMatches` will be stored in a
...
We are currently using the `java.util.logging` package for logging. The `LogsCenter` class is used to manage the logging levels and logging destinations.
