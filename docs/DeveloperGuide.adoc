...
The find feature is facilitated by [blue]`PersonContainsPrefixesPredicate` and [blue]`FindCommand`. +

===== PersonContainsPrefixesPredicate
[blue]`PersonContainsPrefixesPredicate` implements [blue]`Predicate` and is used to test whether a person's attributes
match exactly with at least one of the input keywords in each attribute category.

===== FindCommand
[blue]`FindCommand` extends [blue]`Command` with a [blue]`predicate` that is used to determine whether a given
[blue]`person` should be displayed. It implements [blue]`Command#execute(Model model)` which replaces the
[blue]`predicate` of the current [blue]`model.filteredPersons` with the input predicate of [blue]`ExactFindCommand`.
This triggers a change in the displayed persons to the list of filtered persons.

===== Execution Behaviour
The behaviour of [blue]`FindCommand` is consistent with that of other commands as described in <<Design-Logic>>.
Therefore, this section will gloss over higher-level details and instead zoom-in to the behaviour of
[blue]`FindCommand#execute`.

====== FindCommand Execution
Given below is a step by step description of the behaviour of [blue]`FindCommand#execute`:
...
Step 4: Fuzzy matching uses [blue]`StringUtil#calculateLevenshteinDistance` to filter out possible matches. The matches
are then sorted in ascending order of similarity to input keywords.

Step 5: [blue]`#setDisplayedPersonList` is called.

The following sequence diagram illustrates steps 1 to 5: +

image::ExactFindSequenceDiagram.png[]

NOTE: The above diagram heavily simplifies the execution of [blue]`#fuzzyMatch` as it is further explained in the
section below.

====== Fuzzy Matching
[blue]`FindCommand#execute` performs fuzzy matching between input keywords and person attributes to detect possible
matches. This behaviour is facilitated by
[blue]`FindCommand#fuzzyMatch(ArgumentMultimap argMultimap, List<Person> inputList)`. The behaviour of
[blue]`#fuzzyMatch` is detailed in the below activity diagram:

image::FuzzyMatchActivityDiagram.png[]

As seen from the above diagram, [blue]`#fuzzyMatch` traverses the [blue]`inputList` of persons and tags each person with a
calculated minimum [blue]`combinedLevenshteinDistance`(LD). In the event that no keywords are present, all persons are
...
** Cons: Creation of requisite [blue]`costMatrix` is computationally expensive and may affect performance for long
keywords.
