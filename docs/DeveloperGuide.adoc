...
=== Purpose
This document is written to document the software design and and decisions made in the implementation of ORGANice.

=== Intended audience
This developer guide is meant for developers who would like to further develop ORGANice.
...
`MatchedPatient` objects available in the `ObservableList<Person>` object. The code for the method is as follows:
```
