...
Done is done by accessing `Patient` and `Donor` in `ModelManager`,
...
This method check if the result of the cross-matching test. +

If the patient and donor did a cross-matching test and the result of the test is a fail,
then the patient and donor will need to be send back to find new matches and they
...
Then, the patient and donor will have their `Status` set to `not processing` so that it will be able to find a match in ORGANice.
