...
The list feature is implemented using the `ListCommand(Type type)`. The method
`ModelManager#updateFilteredPersonList(Predicate<Person> predicate)` is used on a `filteredPersons` list.

Below is an activity diagram to illustrate what happens when a user uses the list command.
