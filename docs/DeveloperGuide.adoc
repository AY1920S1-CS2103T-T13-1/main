...
image::ListActivityDiagram.png[width=750]
...
If there is a parameter, ORGANice will check if it is a valid `TYPE` input and display the respective list of doctors, patients
or donors that corresponds to the parameter given by the user. If the parameter is invalid, an error message will be displayed in
the Result Box to indicate that the command format is incorrect.
// end::list[]
