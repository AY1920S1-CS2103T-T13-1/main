...
Sorting is done by using `Comparators` in the `Logic` component and makes use of the `SortCommand#sort(String input)` method.
This method only works after a `MatchCommand` is called by the user, because it takes in `listOfMatches` returned
from calling a `MatchCommand`. Importantly, this `listOfMatches` is an `ObservableList`, which cannot be modified.
Therefore, it is first converted into a `SortedList` so that the list can be modified by sorting.
